...
export abstract class AbstractBaseAgent implements BaseAgent {
  ...
  async receiveFeedback(feedback: FeedbackPoint): Promise<void> {
    try {
      ...
      await this.logError(this.createError('Error receiving feedback', 'receiveFeedback', error));
      throw error;
    }
  }
   ...
   */
  async giveFeedback(targetAgent: string, points: number, context: any): Promise<void> {
  ...
  async executeTask(task: Task): Promise<TaskResult> {
    ...
    try {
      ...
      const errorMessage = e instanceof Error ? e.message : 'Unknown error during task execution';
      error = errorMessage;
  ...
  async scheduleTask(task: Task, schedule: Schedule): Promise<void> {
    try {
      ...
      await this.logError(this.createError('Error scheduling task', 'scheduleTask', error));
      throw error;
    }
  }
  ...
  async getTaskPrompt(name: string): Promise<string> {
    try {
      const result = await this.sql.prepare(`
        ...
        LIMIT 1
      `).bind(name).first<{ prompt_content: string }>();
      ...
      await this.logError(this.createError('Error getting task prompt', 'getTaskPrompt', error));
      throw error;
    }
  }
  ...
  async updateTaskPrompt(name: string, content: string): Promise<void> {
    try {
      // Get current version
      const current = await this.sql.prepare(`
        SELECT version
        FROM task_prompts
        WHERE prompt_name = ?
        ORDER BY version DESC
      ...
      `).bind(name, content, version).run();
    } catch (error) {
      await this.logError(this.createError('Error updating task prompt', 'updateTaskPrompt', error));
      throw error;
    }
  }
  ...
  async useTool<T>(tool: Tool, args: any[]): Promise<T> {
    try {
      ...
      await this.logError(this.createError('Error using tool', 'useTool', error));
      throw error;
    }
  }
  ...
  protected async logTaskStart(task: Task): Promise<void> {
    ...
    `).bind(task.id, this.id, Date.now()).run();
  }
  ...
  protected async logTaskComplete(task: Task, result: any): Promise<void> {
    ...
    `).bind(task.id, this.id, JSON.stringify(result), Date.now()).run();
  }

  protected async logToolUsage(toolName: string, args: any[], result: any): Promise<void> {
    await this.sql.prepare(`
  ...
  protected async updatePromptPerformance(promptName: string, points: number): Promise<void> {
    await this.sql.prepare(`
      UPDATE task_prompts
    ...
    `).bind(points, promptName, promptName).run();
  }

  protected createError(message: string, method: string, context?: any): AgentError {
