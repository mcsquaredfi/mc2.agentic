...
import { shouldEnableAutoGeneratedUI, shouldEnableAIElements, getServerUIConfig } from "../../config/ui-config";

// AI Elements integration (will be set per instance)
let AI_ELEMENTS_AVAILABLE = true;
...
export class ComponentGenerator {
  ...
  constructor(private env: Env) {
    // Initialize AI Elements availability based on server config
    const serverConfig = getServerUIConfig(env);
    AI_ELEMENTS_AVAILABLE = serverConfig.enableAIElements;
  }
  ...
  async generateStyledComponent(
    toolResults: any[],
    context: string
  ): Promise<z.infer<typeof UIComponentSchema> | null> {
    // Check if auto-generated UI is enabled (server-side config)
    const serverConfig = getServerUIConfig(this.env);
    if (!serverConfig.enableAutoGeneratedUI) {
      console.log("🎨 Auto-generated UI is disabled - skipping component generation");
      return null;
    }
